...
type (
	 ...
	 */
	Paises struct {
		Codigo         int        `json:"codigo"           binding:"required" description:"codigo do pais" sql:"type:int" gorm:"primary_key"`
		Nome           string     `json:"nome"             binding:"required" description:"nome do pais" gorm:"not null;type:varchar(32)"`
		InicioVigencia time.Time  `json:"inicioVigencia"   binding:"required" description:"Data de Inicio de Vigencia"  gorm:"type:datetime"`
		FimVigencia    *time.Time `json:"fimVigencia,omitempty" description:"Fim da vigencia" gorm:"type:datetime"`
		Links          []Link     `json:"_links,omitempty" description:"Lista de links" gorm:"-"`
	}

	/**
		Define a estrutura da classe de Resposta padrão. Neste exemplo estamos usando a versão 3.
	 */
	PaisesResposta struct {
		Dados     []Paises   `json:"dados,omitempty"     description:"Lista de Paises"`
		Mensagens []Mensagem `json:"mensagens,omitempty" description:"Lista de Mensagens"`
		Links     []Link     `json:"links,omitempty"     description:"Lista de links"`
		Paginacao Paginacao  `json:"paginacao,omitempty" description:"Dados de Paginacao"`
		Versao	  string	 `json:"versao,omitempty"    description: "Versao do schema de resposta"`
	}
...
func (t Paises) Validate() error {
	...
	if t.InicioVigencia.IsZero() {
		return errors.New("Data de início da vigência não informado!")
	}
 ...
 */
func (t Paises) FindAll(page_num int, page_size int) ([]Paises, error) {
...
/**
	Fixa os links quando e uma lista de paises
 */
func (t *Paises) SetLinksAll() {

	// pega da variável de ambiente o contexto da aplicação
	var context = os.Getenv("API_SRV_CONTEXT")

	// Gera o link quando Paises faz parte de uma consulta de lista.
	t.Links = []Link{Link{
		Nome:   "self",
		Metodo: "GET",
		Tipo:   "application/json",
		Href:   context+"/paises/" + strconv.Itoa(t.Codigo),
		Rel:    "Mostrar",
	},
	}
}
...
/**
	Fixa os links para um pais
 */
func (t *Paises) SetLinks() {

	// pega da variável de ambiente o contexto da aplicação
	var context = os.Getenv("API_SRV_CONTEXT")

	// Gera o link quando Paises faz parte de uma consulta por código, ou seja somente um registro/país.
	t.Links = []Link{Link{
		Nome:   "self",
		Metodo: "GET",
		Tipo:   "application/json",
